...
- **Alembic: причина падений найдена, флаг пока стоит.** Прод работает через
  `DB_USE_LEGACY_MIGRATIONS=1`. В v4.11.0 найдено, почему включение дважды
  роняло прод: в боевой БД таблица `alembic_version` существует, но пуста —
  Alembic создаёт её до записи ревизии, и после упавшего первого `upgrade`
  таблица осталась без строки. Старый auto-stamp проверял только наличие
  таблицы, выходил, и `upgrade` пытался создать существующие таблицы.
  Починено: auto-stamp смотрит на запись, а `init_db_with_fallback` при любом
  сбое миграций откатывается на `init_db()` вместо падения старта. Проверено
  на копии боевой БД. Снимать флаг — отдельное решение: сначала выкатить
  версию с fail-safe и посмотреть логи.
