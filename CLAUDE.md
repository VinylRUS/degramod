...
| `DB_USE_LEGACY_MIGRATIONS` | нет | `1` → миграции через `init_db()` вместо Alembic. На проде снят 20.08.2026 — работает Alembic |
...
- `commands.py` — реестр команд (v5.1.0): паттерны, права, тексты /help.
  ...
  из него. Здесь же `unwarn_args` — разбор двусмысленного «/unwarn N» — и
  `NO_USER_TARGET`: команды, чья цель канал, а не участник (v5.3.0), для
  них `_resolve_punishment_target` не запускается вовсе.
...
### Сообщения от имени каналов — инвариант «своих не удаляем» (v5.3.0)

Юзер может писать в группу от имени своего канала (`message.sender_chat`).
Для спамера это обход персональных ограничений: замьютили человека — он
продолжает от имени канала. `_ChannelMessageMiddleware` удаляет такие
сообщения, если в чате включён `chat_settings.delete_channel_messages`
(по умолчанию **выключен** — цена ошибки тут массовое удаление).

**Инвариант. `_channel_guard_reason` возвращает причину пропуска ДО
всякой проверки тумблера и белого списка, и эти ветки нельзя отменить
ничем:**

1. `sender_chat` не задан — фильтру нечего делать.
2. `sender_chat.id == chat.id` — **сама группа**, то есть её анонимные
   админы. Удаляя их, бот съедал бы мод-команды собственных модераторов.
3. `sender_chat.id == linked_chat_id` — **связанный канал** чата
   обсуждений: и его посты, и комментарии от имени канала-владельца.
4. `is_automatic_forward` — автопост канала. Вместе с ним из чата
   исчезла бы вся ветка комментариев.

Защита сделана **хардкодом, а не записью в `channel_whitelist`** —
намеренно: запись удаляется кнопкой в веб-панели, guard удалить нечем.
Не «упрощай» условие до «нет в списке — удалить». Сторожит
`tests/test_v530_channel_whitelist.py::TestInvariantHoldsWithEmptyWhitelist`
— злой случай «тумблер включён, список пуст».

Прочее по теме:

- Фильтр стоит **outer-middleware**, а не хендлером: `_DisabledChatMiddleware`
  → `_ChannelMessageMiddleware` → `_ReplyContextMiddleware`. Порядок
  объявления хендлеров в этом файле дважды ронял прод (`!ban`, `!alarm`
  — changelog v5.1.0), а middleware от этого класса ошибок не зависит.
- `linked_chat_id` кэшируется в памяти на час (`_linked_chat_cache`).
  Сбой `get_chat` кэшируется тоже — иначе обычная группа без привязки
  дёргала бы Bot API на каждое сообщение от имени канала.
- Матч по `channel_id` (он неизменен). `channel_username` — запасной
  путь для упреждающего внесения; при первой встрече канала id
  дописывается, как `BotWhitelist` делает с `bot_id`.
- Любой сбой проверки — пропуск сообщения, а не удаление «на всякий
  случай»: ошибка в сторону молчания дешевле ошибки в сторону цензуры.
...
- **Alembic включён на проде с 20.08.2026.** До этого прод работал через
  `DB_USE_LEGACY_MIGRATIONS=1`, потому что включение дважды роняло старт.
  Причина найдена в v4.11.0: в боевой БД таблица `alembic_version`
  существовала, но была пуста — Alembic создаёт её до записи ревизии, и после
  упавшего первого `upgrade` таблица осталась без строки. Старый auto-stamp
  проверял только наличие таблицы и выходил, а `upgrade` затем пытался
  создать существующие таблицы. Починено: auto-stamp смотрит на запись, а
  `init_db_with_fallback` при любом сбое миграций откатывается на `init_db()`
  вместо падения старта. Флаг остаётся рабочим рубильником: поставить `1` —
  принудительно старый путь.
