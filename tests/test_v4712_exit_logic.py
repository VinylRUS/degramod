...
# Было `get_event_loop().run_until_complete(...)`. В Python 3.12+ get_event_loop()
# больше не создаёт цикл неявно, а в 3.14 бросает RuntimeError, если своего цикла
# в потоке нет. asyncio.run() делает то же самое и закрывает цикл за собой.
_asyncio.run(_db.init_db())
