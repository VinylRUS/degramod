...
class TestV472Toggles(unittest.IsolatedAsyncioTestCase):
    ...
    # ── Test 3: Миграция сохраняет night_mode_enabled между рестартами (v4.7.23) ─
    async def test_migration_resets_night_mode_enabled(self):
        """v4.7.23: после init_db (повторный запуск — колонка sanitary_days_enabled
        уже добавлена), night_mode_enabled должен СОХРАНЯТЬСЯ (не сбрасываться).

        До v4.7.23: миграция v4.7.2 сбрасывала night_mode_enabled=0 при каждом
        рестарте, если в БД была хоть одна запись с night_mode_enabled=1.
        v4.7.23 фикс: reset только при действительной первой миграции (колонка
        ещё не существует). На обычных рестартах только runtime-флаги
        currently_active сбрасываются.
        """
        async with async_session() as s:
            cs = (await s.execute(
            ...
            self.assertTrue(cs.night_mode_enabled,
                            "v4.7.23: night_mode_enabled должен СОХРАНЯТЬСЯ между "
                            "рестартами (не сбрасываться). Если этот тест падает — "
                            "вернулся баг v4.7.2 'toggle сбрасывается при каждой "
                            "перезагрузке'.")
            # Runtime-флаги сбрасываются на обычном рестарте (stale state cleanup)
            self.assertFalse(cs.night_mode_currently_active,
                             "Migration should reset night_mode_currently_active (runtime state)")
            self.assertFalse(cs.sanitary_days_currently_active,
                             "Migration should reset sanitary_days_currently_active (runtime state)")
