...
# Файлы, которые пока не проходят. Сюита писалась под код версий v4.4–v4.8.10
# и местами отстала от рефакторингов (переименованные функции, версии в
# ассертах, изменившиеся тексты шаблонов). Пока файл в этом списке, его падение
# не роняет прогон — но если он вдруг проходит, раннер об этом сообщает и
# требует убрать строку. Список должен только сокращаться.
