...
# Сколько дней храним снимки. Наказание за сообщение недельной давности —
# исчезающая редкость, а таблица растёт быстрее всех остальных в базе:
# под живым трафиком это ~5400 реплаев и ~1.5 МБ в сутки.
# v5.3.1: 30 → 7 дней — за четверо суток после выката набежало 22 тысячи строк.
_REPLY_CONTEXT_TTL_DAYS: int = 7
