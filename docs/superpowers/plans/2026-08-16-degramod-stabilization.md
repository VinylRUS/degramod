...
| 4 | Обработка flood control | ⚠️ частично | v4.8.7 | `2d41e1b` |
| 5 | Фоновые задачи не теряются | ✅ сделано | v4.8.7 | `2d41e1b` |
| 6 | Убрать блокировку event loop | ⚠️ частично | v4.8.7 | `2d41e1b` |
...
| 10 | Разделить `create_app` | ⚠️ частично | v4.8.9 / v4.8.10 | `b6fe6eb`, `786d9ae` |
...
**Task 10 — частично.** Из 54 роутов в `web/` вынесено 7. Остальные 47 живут
внутри `create_app()` — 3998 строк из 4135. Пересчитано 19.08.2026 на v4.9.0;
в исходной редакции стояло «52 и 45», что не сходилось ни с кодом, ни между
собой. Пофайловая разбивка — в спеке
`docs/superpowers/specs/2026-08-19-web-app-decomposition-design.md`.
...
### Task 4: Обработка flood control Telegram ⚠️ ЧАСТИЧНО (v4.8.7, `2d41e1b`)
...
### Task 6: Убрать блокировку event loop в веб-панели ⚠️ ЧАСТИЧНО (v4.8.7, `2d41e1b`)
...
### Task 10: Разделить `create_app` ⚠️ ЧАСТИЧНО (v4.8.9 `b6fe6eb` / v4.8.10 `786d9ae`)

> В `web/` вынесено 7 роутов из 54. Оставшиеся 47 расписаны в спеке
> `docs/superpowers/specs/2026-08-19-web-app-decomposition-design.md`
> (составлена 19.08.2026, реализация не начата).

`web_app.py:772` — одна функция на 4135 строк, все роуты в замыкании на `bot`
и `templates`. Замер на v4.9.0 (`4e3d5b2`); указанные в исходной редакции плана
`web_app.py:588` и «4070 строк» устарели — файл с тех пор рос.

Роуты выносятся в `web/` по предметным областям, `bot` и `templates`
передаются через `app.state` + `Depends` вместо замыкания. `create_app`
остаётся сборщиком на ~137 строк. Каждый вынесенный роутер получает тест на
«страница отвечает 200 и содержит ключевой элемент» до переноса.

Спека уточняет план в трёх местах, где он был написан «на уровне подхода»:

- каталог — `web/`, а не `routers/`: пакет уже существует с v4.8.9, и в нём
  лежат 7 вынесенных роутов;
- вместо восьми доменов из плана — десять модулей: `admin/settings` и
...
- добавлено правило позднего связывания хелперов (`import web_app` вместо
  `from web_app import ...`) — иначе ломаются два теста в
  `test_v45_dashboard.py`, патчащие `web_app._fetch_and_save_avatar`.
