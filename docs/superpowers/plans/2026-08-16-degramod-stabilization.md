...
| 18 | Сюита: проверки, не роняющие pytest | ⚠️ частично | v4.10.0 | — |
...
### Task 18: Сюита — проверки, не роняющие pytest ⚠️ ЧАСТИЧНО (v4.10.0)
...
`test_v4852_command_prefix`, `test_v488_smoke`.
...
`assert`). Оставшиеся 8 файлов (4 из категории (а), 4 из (б)) не тронуты —
переписать их на общие pytest-фикстуры с настоящими `assert` — отдельная
задача, не входящая в этот план.
