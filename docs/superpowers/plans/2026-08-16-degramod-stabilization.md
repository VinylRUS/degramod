...
| 12 | Alembic вместо ручных миграций | ✅ причина найдена и починена | v4.8.9 / v4.11.0 | `b6fe6eb`, — |
...
### Task 12: Alembic вместо ручных миграций ✅ ПОЧИНЕНО (v4.8.9 `b6fe6eb` / v4.11.0)

> Причина падений найдена 20.08.2026 на копии боевой БД и устранена.
> Флаг `DB_USE_LEGACY_MIGRATIONS=1` на проде пока не снят — см. ниже.

**Причина.** В боевой базе таблица `alembic_version` существует, но **пуста**.
Auto-stamp проверял только наличие таблицы:
...
Видел таблицу — считал базу размеченной — выходил. Alembic дальше читал
ревизию, получал пусто, решал, что не применена ни одна миграция, и запускал
baseline с нуля: `CREATE TABLE` на существующих таблицах →
`OperationalError: table automute_counters already exists` → бот не стартовал.

Как база пришла в это состояние: Alembic создаёт `alembic_version` **до**
записи ревизии. Первая попытка в v4.8.9 упала на существующих таблицах —
таблица осталась, запись нет. Хотфиксы v4.8.9.1/v4.8.9.2 чинили ветку
«таблицы нет вовсе» и этот случай не покрыли.

**Что сделано в v4.11.0:**

1. Auto-stamp проверяет наличие **записи**, а не таблицы. Пустая таблица →
   `stamp head`. Ревизия, неизвестная репозиторию (бывает при откате кода
   назад) → тоже `stamp head`, иначе `upgrade` кинул бы `CommandError`.
2. `init_db_with_fallback` получил настоящий fallback. Раньше функция так
   называлась, но запасного пути не имела: любое исключение из миграций
   поднималось наверх и валило старт. Теперь при сбое — громкий лог в
   logger и stderr, затем `init_db()`. Худший исход миграций — запись в
   логе, а не лежащий бот.

**Проверено на копии боевой БД** (320 наказаний, 163 пользователя): старт
без флага проходит, `alembic_version` заполняется, данные и схема не
меняются, повторный старт идемпотентен, приложение отвечает на `/healthz`,
`/login`, `/dashboard`.

**Флаг снят на проде 20.08.2026.** Владелец выставил `DB_USE_LEGACY_MIGRATIONS=0`,
перезапустил бота — старт прошёл штатно, миграции идут через Alembic. Задача
закрыта полностью: прод переведён на Alembic, ручные миграции в `init_db()`
остались запасным путём на случай сбоя.
