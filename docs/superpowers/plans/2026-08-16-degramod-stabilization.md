...
| # | Задача | Статус | Кем |
|---|---|---|---|
| 1 | Перевод на uv и pyproject | ✅ сделано | эта ветка |
| 2 | Тестовый харнесс | ✅ **перевыполнено** | эта ветка |
| 3 | CI | ✅ **перевыполнено** | эта ветка |
| 4 | Обработка flood control | ⚠️ частично | v4.8.7 |
| 5 | Фоновые задачи не теряются | ✅ сделано | v4.8.7 |
| 6 | Убрать блокировку event loop | ⚠️ частично | v4.8.7 |
| 7 | Срок жизни сессии | ✅ сделано | v4.8.7 |
| 8 | Rate-limit логина не обходится заголовком | ✅ сделано | v4.8.8 |
| 9 | CSRF-токены | ✅ сделано | v4.8.8 |
| 10 | Разделить `create_app` | ⚠️ частично | v4.8.9/v4.8.10 |
| 11 | Разделить `handle_group_command` | ✅ сделано | v4.8.9/v4.8.10 |
| 12 | Alembic вместо ручных миграций | ❌ откачено | v4.8.9 |
| 13 | Привести `roadmap.md` в соответствие | ❌ не сделано | — |
| 14 | `/mywarns` | ✅ сделано + починено | v4.8.10 / эта ветка |
| 15 | Перенос `_alarm_auto_off_tick` | ✅ сделано | v4.8.9 |
| 16 | `/health` → `/healthz` | ❌ не сделано | — |
| 17 | Обновить FastAPI | ❌ не сделано | — |
...
**Task 10 — частично.** Из 52 роутов в `web/` вынесено 7. Остальные 45 живут
внутри `create_app()`.
...
**Task 13 — не сделано.** `roadmap.md` по-прежнему ссылается на
`scripts/test_*.py` в 8 местах (каталог переехал в `tests/`), и расхождения из
таблицы ниже не размечены.

**Task 16 — не сделано.** `/health` отдаёт прежние поля; `memory_mb`,
`memory_percent`, `telegram_connected`, `telegram_api_latency_ms` и градации
`ok/degraded/down` не добавлены.

**Task 17 — не сделано.** FastAPI по-прежнему 0.115.6, `DeprecationWarning`
на месте. Теперь под сюитой обновляться безопаснее, чем было.
...
### Task 2: Тестовый харнесс ✅ ПЕРЕВЫПОЛНЕНО
...
### Task 4: Обработка flood control Telegram ⚠️ ЧАСТИЧНО (v4.8.7)

> `tg_safe_call` есть, покрыто 22 call site из сотен.
...
### Task 5: Фоновые задачи не теряются ✅ ВЫПОЛНЕНО (v4.8.7)
...
### Task 6: Убрать блокировку event loop в веб-панели ⚠️ ЧАСТИЧНО (v4.8.7)

> sqlite3/VACUUM/copy2 вынесены в `to_thread`; блокирующий `open()` остался
> в `web_app.py:738, 3341`.
...
### Task 7: Срок жизни сессии ✅ ВЫПОЛНЕНО (v4.8.7)
...
### Task 8: Rate-limit логина не обходится заголовком ✅ ВЫПОЛНЕНО (v4.8.8)
...
### Task 9: CSRF-токены на изменяющих запросах ✅ ВЫПОЛНЕНО (v4.8.8)
...
### Task 10: Разделить `create_app` ⚠️ ЧАСТИЧНО (v4.8.9/v4.8.10)

> В `web/` вынесено 7 роутов из 52.
...
### Task 11: Разделить `handle_group_command` ✅ ВЫПОЛНЕНО (v4.8.9/v4.8.10)
...
### Task 12: Alembic вместо ручных миграций ❌ ОТКАЧЕНО (v4.8.9)
...
### Task 13: Привести `roadmap.md` в соответствие с кодом ❌ НЕ СДЕЛАНО

> В `roadmap.md` осталось 8 ссылок на `scripts/test_*.py` (каталог теперь `tests/`).
...
### Task 14: `/mywarns` (Issue #6, roadmap v4.9.0 §2.1) ✅ ВЫПОЛНЕНО (v4.8.10) + ПОЧИНЕНО
...
### Task 15: Перенос `_alarm_auto_off_tick` (Issue #15, roadmap v4.9.0 §1.1) ✅ ВЫПОЛНЕНО (v4.8.9)
...
### Task 16: Довести `/health` до `/healthz` (roadmap v5.0.0-07) ❌ НЕ СДЕЛАНО
...
### Task 17: Обновить FastAPI ❌ НЕ СДЕЛАНО

> Под сюитой обновляться безопаснее, чем было на момент написания плана.
