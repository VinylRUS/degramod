...
**Таблица выше неполна — уточнено по факту Task 3.** Task 3 шла с пометкой
«grep-проверок нет», их оказалось две (`test_v481_command_reform.py`). Причина:
первичный анализ ловил только форму `with open(...)` и пропускал обёртки вроде
`_read_file(_WEB_APP_PATH)`. Фактических файлов-кандидатов девять:
`test_v4711_sanitary_roundtrip`, `test_v4724_via_bot_filter`,
`test_v4727_manual_ban_report`, `test_v4728_bot_own_ban_ignore`,
`test_v4729_ux_simplifications`, `test_v475_wordfilter_linkallowlist_ui`,
`test_v476_sanitary_ui_cleanup`, `test_v481_command_reform`, `test_v487_sanity`.

Поимённое предсказание «какой тест сломается на каком домене» ненадёжно и не
требуется. Работающее правило: прогнать сюиту после переноса, переадресовать
покрасневшее по прецеденту Task 2 (`abdfa0f`) и Task 3 (`41fbbf9`).

**Вторая ловушка того же класса: `async_session`.** Тесты с тестовой БД патчат
его в каждом модуле-импортёре. Новый `web/<домен>.py` делает
`from db import async_session` — свой символ, до которого патч
`web_app.async_session` не достаёт, и роут молча идёт в боевую БД. Симптом:
тест падает на данных, а не на импорте. Лечится добавлением
`patch.object(_<домен>, "async_session", ...)` в тест — это практика проекта,
описанная в `tests/known_failing.txt`.
